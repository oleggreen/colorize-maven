...
color_maven() {
  ...
  maven $* | sed \
    -e "s/Tests run: \([1-9][0-9]*\)/${GRN} Tests run: \1$NONE/g" \
    -e "s/, Failures: \([1-9][0-9]*\)/,$RED Failures: \1$NONE/g" \
    -e "s/, Errors: \([1-9][0-9]*\)/,$RED Errors: \1$NONE/g" \
    -e "s/, Skipped: \([1-9][0-9]*\)/,$YEL Skipped: \1$NONE/g" \
